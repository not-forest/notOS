...
use osPrimitives::FI_PREVENTION_PATTERN as freeMutex;
...
impl<T> Mutex<T> {
    ...
    /// Locks the resource and returns the mutex guard, if not taken already.
    ///
    ...
    pub unsafe fn force_unlock(&self) {
        self.status.store(freeMutex, Ordering::SeqCst);
    }
